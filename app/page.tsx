...
const features = [
  ...
  },
  {
    ...
    summary: 'Features only matter when they improve ownership.',
    href: '/topics/product-coverage'
  },
  {
    title: 'The First-Time Buyer Problem',
    summary: 'Better questions before spending real money.',
    ...
    <>
      <section className="section-shell relative overflow-hidden">
        ...
        <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr] lg:items-end">
          ...
          <div className="section-shell border-gold/25 bg-gradient-to-b from-violet/50 to-charcoal/80">
            <p className="kicker">Explore the coverage</p>
            <div className="mt-4 grid gap-2 text-sm">
              {topics.slice(0, 4).map((topic) => (
                <Link
                  key={topic.slug}
                  href={`/topics/${topic.slug}`}
                  className="group rounded-xl border border-white/10 bg-ink/35 p-3 transition hover:border-gold/70 md:p-4"
                >
                  <span className="block text-base font-semibold text-white group-hover:text-gold md:text-lg">{topic.title}</span>
                  <span className="mt-1 block text-sm text-gold/80">{topic.label}</span>
                  <span className="mt-2 block text-sm text-mist/60">View section →</span>
                </Link>
              ))}
            </div>
          </div>
      ...
      <section className="space-y-3">
        ...
        <div className="grid gap-3 md:grid-cols-2">
          {audiences.map((item) => (
            <article key={item} className="editorial-card py-4 md:py-6">
              <h3 className="text-lg font-medium text-white md:text-xl">{item}</h3>
            </article>
          ))}
        </div>
      </section>
        ...
        <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
          {features.map((feature) => (
            <Link key={feature.title} href={feature.href} className="editorial-card block space-y-2">
              <p className="text-lg font-semibold leading-tight text-white md:text-xl">{feature.title}</p>
